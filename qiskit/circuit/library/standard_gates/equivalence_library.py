...
phi = Parameter('phi')
lam = Parameter('lam')
u_to_rxrz = QuantumCircuit(q, global_phase=(lam + phi) / 2)
u_to_rxrz.rz(lam, 0)
u_to_rxrz.rx(pi/2, 0)
...
def_cu.cx(0, 1)
def_cu.u(-theta / 2, 0, -(phi + lam) / 2, 1)  # TODO use QuantumCircuit.u but fails a test
def_cu.cx(0, 1)
def_cu.u(theta / 2, phi, 0, 1)  # TODO use QuantumCircuit.u but fails a test
